...
async def test_started_from_storage_is_preferred_over_from_scratch(storage, handler, body, expected, looptime):
    # TODO: freeze_time is useless here now, since we switched to the loop time.
    #       but we check for the real UTC timestamp, which are UTC-clock-dependent.
    #       => so we have to sync both the loop time & the frozen time (somehow).
    #          and abandon the TSx timestamps concept completely.

    patch = Patch()
    state = State.from_storage(body=Body(body), handlers=[handler], storage=storage)
    with freezegun.freeze_time() as ft:  # for some different time
        ft.tick(123)
...
@pytest.mark.parametrize('expected, body', [
    ...
    (1000, {'status': {'kopf': {'progress': {'some-id': {'delayed': TS0_ISO}}}}}),
    (1001, {'status': {'kopf': {'progress': {'some-id': {'delayed': TS1_ISO}}}}}),
    (999.876544, {'status': {'kopf': {'progress': {'some-id': {'delayed': TSB_ISO}}}}}),
    (1000.876543, {'status': {'kopf': {'progress': {'some-id': {'delayed': TSA_ISO}}}}}),
])
@freezegun.freeze_time(TS0)
