...
async def test_delayed_handlers_sleep(
    ...
    started_dt = '2020-01-01T00:00:00'
    delayed_dt = delayed_iso
    event_type = None if cause_reason == Reason.RESUME else 'irrelevant'
    event_body = {
        'metadata': {'finalizers': [settings.persistence.finalizer]},
        'status': {'kopf': {'progress': {
            'create_fn': HandlerState.from_storage(dict(started=started_dt, delayed=delayed_dt)).as_in_storage(),
            'update_fn': HandlerState.from_storage(dict(started=started_dt, delayed=delayed_dt)).as_in_storage(),
            'delete_fn': HandlerState.from_storage(dict(started=started_dt, delayed=delayed_dt)).as_in_storage(),
            'resume_fn': HandlerState.from_storage(dict(started=started_dt, delayed=delayed_dt)).as_in_storage(),
        }}}
    }
    cause_mock.reason = cause_reason
