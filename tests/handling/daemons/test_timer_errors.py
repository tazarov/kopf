...
async def test_timer_retried_until_retries_limit(
    ...
    await simulate_cycle(event_object)
    async with trigger:
        await trigger.wait_for(lambda: any("but would" in m for m in caplog.messages))
...
#   extract the PR when the timeout/retries errors are raised preventively.
#   extract the PR where the time is switched to the looptime, not UTC ISO time.
